 ...
 *
 * Modes: --real (default, LLM_* env) / --mock / --vision (adds image task).
 */
...
static void write_fixture(const char *rel, const char *content) {
    char full[1024];
    path_resolve(full, sizeof(full), FIXDIR, rel);
    fs_write_file(full, content, strlen(content));
}
...
static int run_python_fixtures(void) {
    if (system("python3 tools/gen_gaia_fixtures.py " FIXDIR " > gaia_fixtures.log 2>&1") != 0 &&
        ...
        return 0;
    char full[1024];
    path_resolve(full, sizeof(full), FIXDIR, "contact.docx");
    if (fs_read_file(full)) return 1;
    return 0;
}

/* ---- vision task (LLM layer; needs a multimodal model) ---- */
static int vision_task(llm *llm, char **answer_out) {
    char full[1024];
    path_resolve(full, sizeof(full), FIXDIR, "diagram.png");
    ...
    fclose(f);
    char *b64 = base64_encode(png, (size_t)len);
    free(png);
    if (!b64) return -1;
    llm_message msgs[1] = {
