...
static int local_execute(void *impl, const char *tool, const char *args_json, coa_executor_result **result) {
    local_impl *li = impl;
    if (!result)
        return -1;
    *result = NULL;
...
coa_executor *coa_executor_new_local(coa_tool_registry *reg, coa_tool_ctx *tctx, void *snapshot) {
    if (!reg || !tctx)
        return NULL;
...
static int route_execute(void *impl, const char *tool, const char *args_json, coa_executor_result **result) {
    route_impl *ri = impl;
    if (!result)
        return -1;
    *result = NULL;
...
static coa_executor *coa_executor_new_route(coa_executor *inner, const char *target, int is_wsl) {
    if (!inner)
        return NULL;
