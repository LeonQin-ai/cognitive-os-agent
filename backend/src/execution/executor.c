...
typedef struct local_impl {
    tool_registry *reg;
    tool_ctx *tctx; /* borrowed; built by the caller (h_act) */
    snapshot *snap; /* may be NULL */
} local_impl;

static int local_start(void *impl) {
    (void)impl;
    return 0;
}
static int local_stop(void *impl) {
    (void)impl;
    return 0;
}
...
static int local_snapshot(void *impl, char **snapshot_id) {
    local_impl *li = impl;
    if (!li->snap || !snapshot_id)
        return -1;
    /* capture the workspace baseline; the id is the commit label */
    snapshot_capture(li->snap, li->tctx->workspace ? li->tctx->workspace : ".");
    const char *id = snapshot_commit(li->snap);
    if (!id)
        return -1;
    *snapshot_id = xstrdup(id);
...
static int local_restore(void *impl, const char *snapshot_id) {
    ...
    if (!li->snap || !snapshot_id)
        return -1;
    return snapshot_restore(li->snap, snapshot_id);
}

static const executor_ops local_ops = {
    "local", local_start, local_execute, local_stop, local_destroy, local_snapshot, local_restore,
};
...
static int route_start(void *impl) {
    route_impl *ri = impl;
    return executor_start(ri->inner);
}
static int route_stop(void *impl) {
    route_impl *ri = impl;
    return executor_stop(ri->inner);
}
static int route_snapshot(void *impl, char **snapshot_id) {
    route_impl *ri = impl;
    return executor_snapshot(ri->inner, snapshot_id);
}
static int route_restore(void *impl, const char *snapshot_id) {
    route_impl *ri = impl;
    return executor_restore(ri->inner, snapshot_id);
}
static void route_destroy(void *impl) {
    route_impl *ri = impl;
    if (ri->inner)
        executor_free(ri->inner);
    free(ri->target);
    free(ri);
}

static const executor_ops wsl_ops = {
    "wsl", route_start, route_execute, route_stop, route_destroy, route_snapshot, route_restore,
};
static const executor_ops remote_ops = {
    "remote", route_start, route_execute, route_stop, route_destroy, route_snapshot, route_restore,
};
...
static executor *executor_new_route(executor *inner, const char *target, int is_wsl) {
    ...
    if (!ri)
        return NULL;
    ri->inner = inner;
    ri->target = target ? xstrdup(target) : NULL;
...
executor *executor_new_wsl(executor *inner, const char *distro) {
    return executor_new_route(inner, distro, 1);
}

executor *executor_new_remote(executor *inner, const char *host) {
    if (!host || !*host)
        return NULL;
    return executor_new_route(inner, host, 0);
}
