...
static ca_tool_result *shell_exec(const ca_tool *self, const ca_tool_ctx *ctx, const char *args_json) {
    (void)self;
    ...
    if (*out_text && !ca_str_utf8_valid_n(out_text, -1)) {
#if defined(_WIN32)
        converted = oem_to_utf8(out_text);
        if (converted && !ca_str_utf8_valid_n(converted, -1)) {
            free(converted);
            converted = NULL;
        }
#endif
        if (!converted) {
            converted = ca_str_utf8_sanitize(out_text);
        }
    ...
    if (pr->timed_out) {
        ...
        r = ca_tool_result_new(1, out_text);
    }
