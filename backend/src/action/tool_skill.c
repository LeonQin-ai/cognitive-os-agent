...
static coa_tool_result *skill_exec(const coa_tool *self, const coa_tool_ctx *ctx, const char *args_json) {
    (void)self;
    if (!ctx || !ctx->skills)
    ...
    if (!name_j || !cJSON_IsString(name_j)) {
        cJSON_Delete(args);
        return coa_tool_result_new(0, "skill: missing string arg 'name'");
    }
