...
coa_skill_result *coa_skill_execute(coa_skill_registry *r, const char *name, const char *args_json,
    ...
    if (strcmp(s->kind, "prompt") == 0) {
        ...
        if (res) {
            res->ok = 0;
            res->output = coa_strdup("prompt skill: run via /v1/skills/run (needs an LLM backend)");
        }
        return res;
    }
    char pyfile[1024] = "";
...
int coa_skill_registry_load(coa_skill_registry *r, const char *state_root) {
    ...
    for (int i = 0; i < cJSON_GetArraySize(arr); i++) {
        ...
        if (!n || !cJSON_IsString(n))
            continue;
        coa_skill sk = {n->valuestring, (d && cJSON_IsString(d)) ? d->valuestring : "",
