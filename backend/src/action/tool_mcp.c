...
static tool_result *mcp_exec(const tool *self, const tool_ctx *ctx, const char *args_json) {
    ...
    srv_j = cJSON_GetObjectItemCaseSensitive(args, "server");
    tool_j = cJSON_GetObjectItemCaseSensitive(args, "tool");
    ...
    rc = mcp_manager_call(ctx->mcp, server, tool, args_out ? args_out : "{}", &out, &err);
    free(args_out);
    cJSON_Delete(args);
    r =
