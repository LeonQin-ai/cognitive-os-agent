...
char *wasm3_run(const void *wasm, size_t wasm_len, const char *fn_name, const char *args_json) {
    ...
    if (!res) {
        ...
        if (root) {
            cJSON *arr = cJSON_IsArray(root) ? root : cJSON_GetObjectItemCaseSensitive(root, "args");
    ...
    if (res) {
        ...
        snprintf(out, sizeof(out), "{\"ok\":true,\"result\":%lld}", (long long)(uint64_t)ret);
    }
