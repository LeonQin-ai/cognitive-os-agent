...
static const model_entry MODELS[] = {
    {"ollama", "Ollama（本地免费）", "openai", "http://localhost:11434/v1", "llama3", "",
     "本地运行 `ollama serve`，无需 key", "", 1},
    {"llamacpp", "llama.cpp / vLLM（本地免费）", "openai", "http://localhost:8080/v1", "", "",
     "本地已加载模型的 HTTP 端点，无需 key", "", 1},
    {"groq", "Groq（免费额度）", "openai", "https://api.groq.com/openai/v1", "llama-3.3-70b-versatile",
     "console.groq.com 注册免费 key", "LPU 推理，免费额度大", "https://console.groq.com/keys", 0},
    ...
    {"gemini", "Gemini（免费额度）", "openai", "https://generativelanguage.googleapis.com/v1beta/openai",
     "gemini-2.0-flash", "aistudio.google.com 免费 key", "OpenAI 兼容端点", "https://aistudio.google.com/apikey", 0},
    {"mistral", "Mistral（免费额度）", "openai", "https://api.mistral.ai/v1", "mistral-small-latest",
     "console.mistral.ai 免费 key", "La Plateforme 免费档", "https://console.mistral.ai/api-keys", 0},
    {"deepseek", "DeepSeek（低价）", "openai", "https://api.deepseek.com/v1", "deepseek-chat", "platform.deepseek.com",
     "接近免费，OpenAI 兼容", "https://platform.deepseek.com/api_keys", 0},
    {"glm", "GLM 智谱（Coding Plan）", "anthropic", "https://open.bigmodel.cn/api/anthropic", "glm-4.6",
     "bigmodel.cn 的 GLM Coding Plan key", "Anthropic 兼容端点，Coding Plan 套餐直接用",
     "https://open.bigmodel.cn/usercenter/apikeys", 0},
    {"ark", "豆包·火山方舟（Coding Plan）", "anthropic", "https://ark.cn-beijing.volces.com/api/coding",
     "glm-5.3-flash", "console.volcengine.com 的 Coding Plan API key",
     "Coding Plan 专用端点；可选模型：glm-5.3 / glm-5.3-flash / doubao-seed-2.0-lite / doubao-seed-2.1-turbo / "
     "deepseek-v4-flash / deepseek-v4-pro / kimi-k2.7-code / minimax-m3",
...
static const mcp_entry MCPS[] = {
    {"mock-echo", "Mock Echo（演示）", "http://127.0.0.1:9000", "JSON-RPC echo 服务器，验证 MCP 通路", "本地", 1,
     "node tools/mock_mcp_server.js", "", "", "", 0},
    {"filesystem", "FileSystem（本地文件）", "", "读取/写入本地目录（filesystem MCP，stdio）", "本地", 1,
     "npx @modelcontextprotocol/server-filesystem", "", "npx", "-y @modelcontextprotocol/server-filesystem .", 0},
    {"sqlite", "SQLite（本地数据库）", "", "SQLite 数据库读写（sqlite MCP，stdio）", "本地", 1,
     "npx @modelcontextprotocol/server-sqlite", "", "npx", "-y @modelcontextprotocol/server-sqlite", 0},
    {"playwright", "Playwright（浏览器自动化）", "", "浏览器导航/点击/填表（playwright MCP，stdio）", "本地", 1,
     "npx @playwright/mcp", "", "npx", "-y @playwright/mcp", 0},
    /* ---- GitHub 热门 MCP 应用（modelcontextprotocol/servers 官方仓库等） ---- */
    {"github", "GitHub（官方托管）", "https://api.githubcopilot.com/mcp/", "GitHub 仓库/Issue/PR 操作（官方 MCP）",
     "GitHub", 0, "GitHub Personal Access Token", "https://github.com/github/github-mcp-server", "", "", 1},
    {"fetch", "Fetch（网页抓取）", "", "抓取 URL 并转换为可读 Markdown（stdio）", "GitHub", 1,
     "npx @modelcontextprotocol/server-fetch", "https://github.com/modelcontextprotocol/servers/tree/main/src/fetch",
     "npx", "-y @modelcontextprotocol/server-fetch", 0},
    {"memory", "Memory（知识图谱记忆）", "", "基于知识图谱的持久化记忆（stdio）", "GitHub", 1,
     "npx @modelcontextprotocol/server-memory", "https://github.com/modelcontextprotocol/servers/tree/main/src/memory",
     "npx", "-y @modelcontextprotocol/server-memory", 0},
    {"everything", "Everything（全工具测试）", "", "覆盖所有工具类型的 MCP 服务器，用于测试客户端（stdio）", "GitHub",
     1, "npx @modelcontextprotocol/server-everything",
     "https://github.com/modelcontextprotocol/servers/tree/main/src/everything", "npx",
     "-y @modelcontextprotocol/server-everything", 0},
    {"sequential-thinking", "Sequential Thinking（分步推理）", "", "逐步推理工具，帮助模型解决复杂问题（stdio）",
     "GitHub", 1, "npx @modelcontextprotocol/server-sequential-thinking",
     "https://github.com/modelcontextprotocol/servers/tree/main/src/sequentialthinking", "npx",
     "-y @modelcontextprotocol/server-sequential-thinking", 0},
    {"time", "Time（时间/时区）", "", "当前时间与时区转换工具（stdio）", "GitHub", 1,
     "npx @modelcontextprotocol/server-time", "https://github.com/modelcontextprotocol/servers/tree/main/src/time",
     "npx", "-y @modelcontextprotocol/server-time", 0},
    {"puppeteer", "Puppeteer（浏览器自动化）", "", "无头 Chrome 浏览器自动化（官方 puppeteer MCP，stdio）", "GitHub", 1,
     "npx @modelcontextprotocol/server-puppeteer",
     "https://github.com/modelcontextprotocol/servers/tree/main/src/puppeteer", "npx",
     "-y @modelcontextprotocol/server-puppeteer", 0},
    {"notion", "Notion（知识库）", "https://mcp.notion.so/mcp", "Notion 工作区读写（官方 Notion MCP）", "GitHub", 0,
     "Notion Integration Token", "https://github.com/makenotion/notion-mcp-server", "", "", 1},
    {"browserbase", "Browserbase（云端浏览器）", "", "云端无头浏览器自动化/网页抓取（browserbase MCP，stdio）",
     "GitHub", 1, "BROWSERBASE_API_KEY + BROWSERBASE_PROJECT_ID",
     "https://github.com/browserbase/mcp-server-browserbase", "npx", "-y @browserbasehq/mcp", 0},
    /* ---- 参考入口（合集/工具仓库，非单个 server，不可一键连接） ---- */
    {"official-servers", "MCP 官方 Server 合集", "", "modelcontextprotocol/servers 官方参考实现合集（参考入口）",
     "参考", 0, "", "https://github.com/modelcontextprotocol/servers", "", "", 0},
    {"awesome-mcp", "Awesome MCP Servers", "", "社区精选 MCP Server 大全（参考入口）", "参考", 0, "",
     "https://github.com/punkpeye/awesome-mcp-servers", "", "", 0},
    {"mcp-cli", "mcp-cli（MCP 命令行调试）", "", "命令行调用/调试 MCP Server 的工具（参考入口）", "参考", 0, "",
...
char *catalog_models_json(void) {
    ...
    for (int i = 0; i < N_MODELS; i++) {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 i ? "," : "", id, name, prov, base, model, kh, note, su, m->local ? "true" : "false");
...
char *catalog_mcp_json(void) {
    ...
    for (int i = 0; i < N_MCPS; i++) {
        ...
        char id[64], name[128], url[256], desc[600], cat[64], kh[256], repo[300], cmd[256], args[512];
        json_esc(id, sizeof(id), m->id);
        json_esc(name, sizeof(name), m->name);
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 i ? "," : "", id, name, url, desc, cat, m->needs_local ? "true" : "false", kh, repo,
                 (m->command && *m->command) ? "stdio" : ((m->url && *m->url) ? "http" : "reference"), cmd, args,
...
static const catalog_skill SKILLS[] = {
    ...
    {"greet", "Greet（参数化问候）", "按 {{who}} 参数输出问候语", "shell", "echo hello {{who}}", "{\"who\":\"world\"}",
     ""},
    {"banner", "Banner（标题横幅）", "输出 === {{title}} === 横幅行", "shell",
     "echo === {{title}} ===", "{\"title\":\"demo\"}", ""},
    /* ---- 可安装运行（python：需本机 python 命令） ---- */
    {"py_now", "Now（当前时间戳）", "输出 ISO 格式当前时间", "python",
     "import datetime; print(datetime.datetime.now().isoformat())", "", ""},
    {"py_sysinfo", "SysInfo（系统信息）", "输出 OS 与 Python 版本信息", "python",
     "import platform, sys; print(platform.system(), platform.release()); print(sys.version.split()[0])", "", ""},
    {"py_calc", "Calc（数值求和）", "计算 {{a}} + {{b}} 并输出", "python", "print({{a}} + {{b}})", "{\"a\":1,\"b\":2}",
     ""},
    {"py_uuid", "UUID（随机标识）", "生成一个 UUIDv4 随机标识", "python", "import uuid; print(uuid.uuid4())", "", ""},
    {"py_sha256", "SHA256（文本指纹）", "计算 {{text}} 的 SHA-256 摘要", "python",
     "import hashlib; print(hashlib.sha256('{{text}}'.encode()).hexdigest())", "{\"text\":\"cognitive-os-agent\"}", ""},
    /* ---- 可安装运行（prompt：经当前 LLM 后端执行，源自 fabric patterns） ---- */
    {"summarize", "Summarize（内容摘要）", "将给定内容浓缩为一段要点摘要（源自 fabric patterns）", "prompt",
     "Summarize the content below in one clear paragraph, followed by 3 key bullet points.\n\nContent:\n{{content}}",
     "{\"content\":\"Cognitive OS is a C-native runtime for autonomous AI agents. It treats the LLM as an accelerator "
     "rather than the operating system, providing memory, context management, transactions, policy and multi-agent "
     "orchestration.\"}",
     "https://github.com/danielmiessler/fabric"},
    {"extract_wisdom", "Extract Wisdom（要点提炼）", "从文本中提炼核心观点/洞见/事实（源自 fabric patterns）", "prompt",
     "Extract the main ideas, key facts and notable quotes from the content below. Reply as short bullet "
     "lists.\n\nContent:\n{{content}}",
     "{\"content\":\"LLM is the accelerator, not the OS. A cognitive runtime must own memory, context and policy, and "
     "treat model calls as replaceable compute.\"}",
     "https://github.com/danielmiessler/fabric"},
    {"explain_code", "Explain Code（代码讲解）", "逐步解释给定代码的功能（源自 fabric patterns）", "prompt",
     "Explain what the following code does, step by step, in simple terms. Then list any bugs or risks you "
     "notice.\n\nCode:\n{{code}}",
     "{\"code\":\"int f(int n){return n<2?n:f(n-1)+f(n-2);}\"}", "https://github.com/danielmiessler/fabric"},
    {"improve_writing", "Improve Writing（润色改写）", "把给定文本改写得更清晰专业（源自 fabric patterns）", "prompt",
     "Rewrite the text below so it is clear, concise and professional. Keep the original language and "
     "meaning.\n\nText:\n{{text}}",
     "{\"text\":\"this runtime very good, it make agent run fast and safe\"}",
     "https://github.com/danielmiessler/fabric"},
    /* ---- 参考：技能/提示词框架仓库（不可直接运行） ---- */
    {"fabric", "Fabric（提示词技能框架）", "danielmiessler/fabric 模块化 LLM 技能（patterns）精选", "reference", "", "",
     "https://github.com/danielmiessler/fabric"},
    {"anthropics-skills", "Anthropic Skills（官方技能库）",
     "anthropics/skills 官方 Claude 技能合集（docx/pdf/前端设计等）", "reference", "", "",
     "https://github.com/anthropics/skills"},
    {"awesome-cursorrules", "Awesome CursorRules", "PatrickJS/awesome-cursorrules 规则/技能精选列表", "reference", "",
     "", "https://github.com/PatrickJS/awesome-cursorrules"},
    {"skillhub-cn", "SkillHub（skillhub.cn 技能市场）",
     "skillhub.cn 中文 MCP/技能市场 — 浏览后可将技能手动导入本运行时", "reference", "", "", "https://skillhub.cn"},
};
...
static const catalog_remote_skill REMOTE_SKILLS[] = {
    ...
    {"fabric", "fab_summarize", "Fabric: Summarize（内容摘要）",
     ...
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/summarize/"
     "system.md"},
    {"fabric", "fab_extract_wisdom", "Fabric: Extract Wisdom（要点提炼）",
     ...
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/extract_wisdom/"
     "system.md"},
    {"fabric", "fab_improve_writing", "Fabric: Improve Writing（润色）", "fabric patterns 原版写作润色技能",
     "https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/improve_writing/system.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/improve_writing/"
     "system.md"},
    {"fabric", "fab_analyze_claims", "Fabric: Analyze Claims（论断核查）", "逐条分析论断的真值与依据",
     "https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/analyze_claims/system.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/analyze_claims/"
     "system.md"},
    {"fabric", "fab_create_quiz", "Fabric: Create Quiz（出题）", "根据给定内容生成测验题",
     "https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/create_quiz/system.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/create_quiz/"
     "system.md"},
    {"fabric", "fab_rate_content", "Fabric: Rate Content（内容评分）", "多维打分并给出改进建议",
     "https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/rate_content/system.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/rate_content/"
     "system.md"},
    {"fabric", "fab_explain_code", "Fabric: Explain Code（代码讲解）", "逐步解释代码功能与风险",
     "https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/explain_code/system.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/danielmiessler/fabric/main/data/patterns/explain_code/"
     "system.md"},
    /* ---- anthropics/skills — skills/<name>/SKILL.md（官方 Claude 技能库） ---- */
    {"anthropics", "ant_docx", "Anthropic: Docx（Word 文档）", "创建/编辑/分析 Word 文档的官方技能（SKILL.md 原文）",
    ...
    {"anthropics", "ant_pdf", "Anthropic: Pdf（PDF 处理）", "PDF 表单填写/文本提取/拆分合并的官方技能",
     "https://raw.githubusercontent.com/anthropics/skills/main/skills/pdf/SKILL.md",
     "https://ghproxy.net/https://raw.githubusercontent.com/anthropics/skills/main/skills/pdf/SKILL.md"},
    {"anthropics", "ant_frontend_design", "Anthropic: Frontend Design（前端设计）", "官方前端设计品味技能",
    ...
    {"anthropics", "ant_brand_guidelines", "Anthropic: Brand Guidelines（品牌规范）", "官方品牌规范应用技能",
    ...
    {"anthropics", "ant_internal_comms", "Anthropic: Internal Comms（内部通讯）", "官方内部通讯写作技能",
    ...
    {"anthropics", "ant_canvas_design", "Anthropic: Canvas Design（视觉设计）", "官方画布/海报视觉设计技能",
    ...
    {"cursorrules", "cr_ts_vuejs", "CursorRules: TypeScript + Vue.js", "awesome-cursorrules 精选规则原文（.mdc）",
     "https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "typescript-vuejs-cursorrules-prompt-file.mdc",
     "https://ghproxy.net/https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "typescript-vuejs-cursorrules-prompt-file.mdc"},
    {"cursorrules", "cr_angular_ts", "CursorRules: Angular + TypeScript", "Angular 开发规范规则原文",
     "https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "angular-typescript-cursorrules-prompt-file.mdc",
     "https://ghproxy.net/https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "angular-typescript-cursorrules-prompt-file.mdc"},
    {"cursorrules", "cr_cpp", "CursorRules: C++ 编程规范", "C++ 编码指南规则原文",
     "https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "cpp-programming-guidelines-cursorrules-prompt-file.mdc",
     "https://ghproxy.net/https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "cpp-programming-guidelines-cursorrules-prompt-file.mdc"},
    {"cursorrules", "cr_code_guidelines", "CursorRules: 通用代码规范", "通用代码质量规则原文",
     "https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "code-guidelines-cursorrules-prompt-file.mdc",
     "https://ghproxy.net/https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
     "code-guidelines-cursorrules-prompt-file.mdc"},
    {"cursorrules", "cr_anti_overeng", "CursorRules: 反过度工程", "约束 AI 生成代码不过度设计的规则原文",
    ...
    {"cursorrules", "cr_android_compose", "CursorRules: Android Jetpack Compose", "Compose 开发规范规则原文",
     "https://raw.githubusercontent.com/PatrickJS/awesome-cursorrules/main/rules/"
...
const catalog_remote_skill *catalog_remote_skill_find(const char *repo, const char *id) {
    for (int i = 0; i < N_REMOTE; i++) {
        const catalog_remote_skill *e = &REMOTE_SKILLS[i];
        if (repo && strcmp(e->repo, repo) != 0)
            continue;
...
char *catalog_remote_skill_fetch(const catalog_remote_skill *e) {
    if (!e || !e->raw_url || !*e->raw_url)
        return NULL;
    ...
    for (int u = 0; u < 2; u++) {
        ...
        if (split_url(urls[u], &base, &path) != 0)
            continue;
        http_response *r = http_get(base, path, NULL, 10000);
        free(base);
        free(path);
...
int catalog_skill_count(void) {
    return N_SKILLS;
}

const catalog_skill *catalog_skill_at(int i) {
...
char *catalog_skills_json(void) {
    ...
    for (int i = 0; i < N_SKILLS; i++) {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 "\"source\":\"%s\",\"type\":\"%s\"}",
                 i ? "," : "", id, name, desc, kind, body, ta, src, type);
        size_t cur = strlen(out), blen = strlen(buf);
        char *no = realloc(out, cur + blen + 2);
        if (!no)
            break;
