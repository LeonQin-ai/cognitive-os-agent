...
int ca_persist_set_root(const char *dir) {
    ...
    return ca_fs_mkdirs(g_root) == 0 ? 0 : -1;
}

const char *ca_persist_root(void) { return g_root; }

void ca_persist_path(char *out, size_t n, const char *sub, const char *file) {
    if (sub && *sub) {
        ca_path_join(out, n, g_root, sub);
        if (file && *file) ca_path_join(out, n, out, file);
    } else if (file && *file) {
        ca_path_join(out, n, g_root, file);
    } else {
...
int ca_persist_ensure(const char *sub) {
    char p[1024];
    ca_persist_path(p, sizeof(p), sub, NULL);
    return ca_fs_mkdirs(p) == 0 ? 0 : -1;
}

char *ca_persist_read(const char *sub, const char *file) {
    char p[1024];
    ca_persist_path(p, sizeof(p), sub, file);
    return ca_fs_read_file(p);
}

int ca_persist_write(const char *sub, const char *file, const char *text) {
    if (ca_persist_ensure(sub) != 0) return -1;
    char p[1024];
    ca_persist_path(p, sizeof(p), sub, file);
