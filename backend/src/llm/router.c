...
ca_router *ca_router_new(void) {
    ...
    if (!r) return NULL;
    ca_mutex_init(&r->mtx);
    r->policy = ca_strdup("round_robin");
    return r;
}

void route_clear(ca_route *e) {
...
int ca_router_add(ca_router *r, const char *name, const char *provider,
    ...
    return ca_router_add_ex(r, name, provider, base_url, api_key, model,
                            weight, 0, 0, NULL);
}
...
int ca_router_add_ex(ca_router *r, const char *name, const char *provider,
    ...
    for (size_t i = 0; i < r->count; i++) {
        if (strcmp(r->routes[i].name, name) == 0) {
            ...
            e->caps = caps ? ca_strdup(caps) : NULL;
            e->weight = weight;
...
static int route_has_cap(const ca_route *e, const char *tag) {
    if (!e->caps) return 0;
...
static ca_route *pick_locked(ca_router *r) {
    ...
    if (!is_cost && !is_lat && !is_cap) {
        ...
        ca_route *e = &r->routes[r->cursor % r->count];
    ...
    if (is_cap) {
        ...
        if (n == 0) { /* nobody carries the tag: degrade to full round-robin */
            ...
            ca_route *e = &r->routes[cand[r->cursor++ % n]];
            return e;
        }
    } else {
        for (size_t i = 0; i < r->count && n < 64; i++) cand[n++] = i;
    ...
    for (size_t i = 1; i < n; i++) {
        ca_route *a = &r->routes[cand[i]], *b = &r->routes[best];
...
int ca_router_load_file(ca_router *r, const char *path) {
    ...
    cJSON_ArrayForEach(it, arr) {
        ...
        ca_router_add_ex(r, name->valuestring, prov->valuestring,
