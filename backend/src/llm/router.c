...
int coa_router_add(coa_router *r, const char *name, const char *provider, const char *base_url, const char *api_key,
                   const char *model, double weight) {
    return coa_router_add_ex(r, name, provider, base_url, api_key, model, weight, 0, 0, NULL);
}

int coa_router_add_ex(coa_router *r, const char *name, const char *provider, const char *base_url, const char *api_key,
...
int coa_router_remove(coa_router *r, const char *name) {
    ...
    for (size_t i = 0; i < r->count; i++) {
        if (strcmp(r->routes[i].name, name) == 0) {
            route_clear(&r->routes[i]);
            memmove(&r->routes[i], &r->routes[i + 1], (r->count - i - 1) * sizeof(coa_route));
            r->count--;
            coa_mutex_unlock(&r->mtx);
            return 1;
        }
...
int coa_router_load_file(coa_router *r, const char *path) {
    ...
    cJSON_ArrayForEach(it, arr) {
        ...
        coa_router_add_ex(r, name->valuestring, prov->valuestring, (b && cJSON_IsString(b)) ? b->valuestring : NULL,
