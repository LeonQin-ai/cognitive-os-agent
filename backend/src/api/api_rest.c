...
static int h_tools(const http_request *req, http_response *resp, void *ud) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (t->json_schema && *t->json_schema) {
            ...
            if (sc)
                cJSON_AddItemToObject(o, "schema", sc);
            else
                cJSON_AddStringToObject(o, "schema", t->json_schema);
        }
        cJSON_AddItemToArray(arr, o);
    }
...
static int h_plugin_native_load(const http_request *req, http_response *resp, void *ud) {
    ...
    if (pj && cJSON_IsString(pj))
        snprintf(path_buf, sizeof(path_buf), "%s", pj->valuestring);
...
static int h_skill_run(const http_request *req, http_response *resp, void *ud) {
    ...
    if (sk && sk->kind && strcmp(sk->kind, "prompt") == 0) {
        ...
        if (answer) {
            ...
            cJSON_AddStringToObject(o, "output", "LLM unavailable (configure a provider first)");
        }
        free(answer);
...
typedef struct {
    const char *name, *description, *kind, *body;
} skill_tmpl;
...
static const struct {
    const char *name, *desc, *repo, *kind, *winget_id;
} GH_SKILLS[] = {
...
static int h_mcp_add(const http_request *req, http_response *resp, void *ud) {
    ...
    char name_buf[128] = "", tr_buf[32] = "", url_buf[512] = "", tok_buf[512] = "", cmd_buf[256] = "",
         args_buf[512] = "";
    if (root && cJSON_IsObject(root)) {
        cJSON *n = cJSON_GetObjectItemCaseSensitive(root, "name");
        ...
        cJSON *a = cJSON_GetObjectItemCaseSensitive(root, "args");
        if (n && cJSON_IsString(n))
            snprintf(name_buf, sizeof(name_buf), "%s", n->valuestring);
        if (tr && cJSON_IsString(tr))
            snprintf(tr_buf, sizeof(tr_buf), "%s", tr->valuestring);
        if (u && cJSON_IsString(u))
            snprintf(url_buf, sizeof(url_buf), "%s", u->valuestring);
        if (t && cJSON_IsString(t))
            snprintf(tok_buf, sizeof(tok_buf), "%s", t->valuestring);
...
static int h_im_channel_add(const http_request *req, http_response *resp, void *ud) {
    ...
    if (root && cJSON_IsObject(root)) {
        ...
        if (o && cJSON_IsString(o))
            name = o->valuestring;
        o = cJSON_GetObjectItemCaseSensitive(root, "type");
        if (o && cJSON_IsString(o))
            type = o->valuestring;
        o = cJSON_GetObjectItemCaseSensitive(root, "endpoint");
        if (o && cJSON_IsString(o))
            endpoint = o->valuestring;
        o = cJSON_GetObjectItemCaseSensitive(root, "token");
        if (o && cJSON_IsString(o))
            token = o->valuestring;
        o = cJSON_GetObjectItemCaseSensitive(root, "target");
        if (o && cJSON_IsString(o))
            target = o->valuestring;
        o = cJSON_GetObjectItemCaseSensitive(root, "enabled");
        if (o && cJSON_IsBool(o))
...
static int h_im_session_route(const http_request *req, http_response *resp, void *ud) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (msgs[i].sender)
            cJSON_AddStringToObject(o, "sender", msgs[i].sender);
